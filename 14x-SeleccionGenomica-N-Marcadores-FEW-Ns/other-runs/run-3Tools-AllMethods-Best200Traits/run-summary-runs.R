...
main <- function () {
	...
	runsDir = args [1]
    summarizeRuns (runsDir)
}
#-------------------------------------------------------------
#-------------------------------------------------------------
summarizeRuns <- function (runsDir) {
    runsDirs = list.files (runsDir)
	for (runDir in runsDirs) {
        resultsDir = sprintf ('%s/%s/%s/%s', runsDir, runDir, "outputs", 'results')
        print (sprintf ("Summarizing run : %s", resultsDir))
        createSummaryTables (resultsDir)
    }
}
...
#-------------------------------------------------------------
createSummaryTables <- function (resultsDir) {
...
#	phenoNames               = colnames (read.csv (phenotypeFile)[,-1])
	phenoNames               = list.files (resultsDir)
	...
	for (pheno in phenoNames) {
		...
		predictionsFilePath = sprintf ("%s/%s/%s", resultsDir, pheno, fileCrossValPredictions)
		if (!file.exists (predictionsFilePath)) {
			...
			filePathGEBVsPredictions = sprintf ("%s/%s/%s", resultsDir, pheno, fileGEBVsPredictions)
