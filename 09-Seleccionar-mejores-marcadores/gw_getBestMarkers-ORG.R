...
#reload (pkgload::inst("gwascolors"))
	...
	outFile = gsub (".csv", "-09-BASE.csv", markersFile)
	...
	outFile = gsub (".csv", "-09-HCL.csv", markersFile)
    ...
    #---------------- GSCORE > 0.8 -----------------------------------------------
	# Best significative for base traits
	outFile = gsub (".csv", "-08-BASE.csv", markersFile)
	bestByTraitBaseFile = gw_bestMarkersTraits (markersFile, 0.8, "BASE", outFile)
	plotBestSNPsByTrait (bestByTraitBaseFile, "BASE")

	# Best significative for HCL traits
	outFile = gsub (".csv", "-08-HCL.csv", markersFile)
	bestByTraitBaseFile = gw_bestMarkersTraits (markersFile, 0.8, "TRAIT", outFile)
	plotBestSNPsByTrait (bestByTraitBaseFile, "TRAIT")
...
gw_bestMarkersTraits <- function (markersFile, GVALUE, TRAITTYPE, outFile) {
	...
	for (trait in traitNames) {
		...
		bestMarkers    = filter (uniqueMarkers, GSCORE >= GVALUE)
